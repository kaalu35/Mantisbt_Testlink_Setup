...
function layout_navbar() {
	...
	echo '<span style="font-size:20px; font-weight:700; color:#ffffff; letter-spacing:0.3px;">Kasetti</span>';
	echo '</a>';
...
function layout_footer() {
	...
	echo '<address>' . "\n";
	echo '<strong>Powered by <a href="' . config_get_global( 'path' ) . '" title="Kasetti Technologies">Kasetti</a></strong> <br>' . "\n";
	...
	if( !is_page_name( 'login_page' ) ) {
		...
		$t_company_logo_url = helper_mantis_url( 'images/kasetti-footer.png' );
		echo '<a href="' . config_get_global( 'path' ) . '" title="Kasetti Technologies">' .
			'<img src="' . $t_company_logo_url . '" width="190" height="62" ' .
			'alt="Kasetti Technologies" />' .
			'</a>' . "\n";
		echo '</div>' . "\n";
		echo '</div>' . "\n";
	}
